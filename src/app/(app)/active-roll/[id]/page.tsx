...
export default async function ActiveRollShootPage({
  ...
  const [{ data: camera }, { data: activeLenses }, { data: filmStock }, { data: lastFrame }] =
    await Promise.all([
  ...
  // The default lens (or, if a frame exists, the lens it used) must always be
  // representable even if it's since been deactivated — otherwise the
  // shooter would silently show no lens selected for gear that's actually
  // mounted.
  let lenses: Lens[] = activeLenses ?? [];
  const requiredLensId = lastFrame?.lens_id ?? roll.default_lens_id;
  ...
  const nextFrameNumber = (lastFrame?.frame_number ?? 0) + 1;
