...
/**
 * Manually edits a roll's start/end date — only meaningful once shooting is
 * done (an active roll's end_date is set by finishing it, not typed in).
 * The generated title has never been stored anywhere; it's computed fresh
 * from these columns on every render, so changing a date here is all that's
 * needed for the displayed title to update — there's no separate "title"
 * field to keep in sync.
 */
export async function updateRollDates(
  rollId: string,
  startDate: string,
  endDate: string | null,
