  ...
  return (
    <div className="flex flex-col items-end gap-1">
      <button
        ...
        className="rounded-md border border-border px-2.5 py-1 text-xs text-muted-strong hover:border-fd-red hover:text-fd-red disabled:opacity-50"
      >
        {pending ? "Deleting…" : "Delete"}
      </button>
      {error && <p className="max-w-48 text-right text-xs text-fd-red">{error}</p>}
    </div>
  );
}
